...
The resulting dataset has `17,516` observations. There are `r length(unique(zorimerged_df$zip_code[!is.na(zorimerged_df$zip_code)]))` unique zip codes and `r length(unique(zorimerged_df$neighborhood[!is.na(zorimerged_df$neighborhood)]))` neighborhoods.
